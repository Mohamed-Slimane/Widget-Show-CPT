...
* Plugin URI: 
* Description: A sidebar Widget for displaying the latest posts of any post type including custom types. Control the widget title formatting and the number of posts to display. Plus the widget is completely localized for other languages.
* Version: 1.1
* Author: Jose Lazo
...
* License: GPL
*/
function jjlazo_latest_cpt_init() {
	if ( !function_exists( 'register_sidebar_widget' ))
return;

function jjlazo_latest_cpt($args) {
	global $post;
	...
	$options = get_option( 'jjlazo_latest_cpt' );
	...
	// save buffer content into a variable
...
} // end function jjlazo_latest_cpt


/**
* Widget settings form function
*/
function jjlazo_latest_cpt_control() {
	...
	if ( $_POST['latest-cpt-submit'] ) {
		...
		update_option( 'jjlazo_latest_cpt', $options );
	}
	...
	<?php
} // end fucntion jjlazo_latest_cpt_control


wp_register_sidebar_widget( 'widget_latest_cpt', __('Latest Custom Posts'), 'jjlazo_latest_cpt' );
wp_register_widget_control( 'widget_latest_cpt', __('Latest Custom Posts'), 'jjlazo_latest_cpt_control', 300, 200 );

add_action( 'widgets_init', 'jjlazo_latest_cpt_init' );
